...
const GMAIL_PROCESSOR_VERSION=5;
...
const syncGoogle=async(u:string)=>{
 ...
 let scanned=0,created=0,duplicates=0,fetchFailures=0,matched=0,verifiedCancellations=0,promotional=0,active=0,possible=0,high=0,medium=0,low=0,incrementalCandidates=0,searchCandidates=0,capped=false;
 const modes=new Set<string>();
 for(const rec of records){
  const before={scanned,matched,created,duplicates,fetchFailures,promotional,active,possible,high,medium,low,incrementalCandidates,searchCandidates},connectionModes=new Set<string>();
  let connectionCapped=false;
  const recordMode=(value:string)=>{modes.add(value);connectionModes.add(value)},markCapped=()=>{capped=true;connectionCapped=true};
  ...
  const allSeen=(await db.list<any>(table(u,'gmail-sources'),{limit:500})).items,seen=allSeen.filter(x=>!x.providerAccountIdentifier||x.providerAccountIdentifier===rec.providerAccountIdentifier),seenById=new Map(seen.map(x=>[x.messageId,x])),hasMatched=seen.some(x=>x.matched),needsClassifierMigration=Number(checkpoint?.processorVersion||0)<GMAIL_PROCESSOR_VERSION;
  ...
  if(checkpoint?.historyId){
   ...
   for(let page=0;page<3&&ids.size<GMAIL_CANDIDATE_CAP;page++){
    ...
    if(historyResponse.status===404){checkpoint.historyId='';needsFullSearch=true;recordMode('checkpoint_fallback');break}
    ...
    const data=await historyResponse.json() as {history?:Array<{messagesAdded?:Array<{message:{id:string}}>}>;nextPageToken?:string};
     for(const event of data.history||[])for(const added of event.messagesAdded||[]){if(ids.size<GMAIL_CANDIDATE_CAP)ids.add(added.message.id);else markCapped()}
     pageToken=data.nextPageToken||'';if(!pageToken)break;if(page===2)markCapped();
   }
   if(ids.size){incrementalCandidates+=ids.size;recordMode('incremental')}
  }
  if(needsFullSearch||!hasMatched||needsClassifierMigration){
   recordMode(needsFullSearch?'initial_search':needsClassifierMigration?'classifier_migration_search':'validation_search');
   const queries=[
    ...
    'newer_than:2y {subject:"start your free trial" subject:"try premium free" subject:"months free" subject:"free phone line" subject:"free mobile line" subject:"claim your offer" subject:"upgrade today" subject:"special offer"}',
    ...
    for(const query of queries){
     if(ids.size>=GMAIL_CANDIDATE_CAP){markCapped();break}
     let pageToken='';
     for(let page=0;page<2&&ids.size<GMAIL_CANDIDATE_CAP;page++){
      ...
      const before=ids.size;for(const item of data.messages||[]){if(ids.size<GMAIL_CANDIDATE_CAP)ids.add(item.id);else markCapped()}searchCandidates+=ids.size-before;
      pageToken=data.nextPageToken||'';if(!pageToken)break;if(page===1)markCapped();
     }
   }
  }
  const existingTrials=await listTrials(u);
   ...
   for(const id of Array.from(ids).slice(0,GMAIL_CANDIDATE_CAP)){
    ...
    if(Number(prior?.processorVersion||0)>=GMAIL_PROCESSOR_VERSION){duplicates++;continue}
   ...
   for(let i=0;i<candidateIds.length;i+=CHUNK_SIZE){
    ...
    for(const item of batch){
     ...
     if(evidence){
      ...
      const sourceRecord={providerAccountIdentifier:rec.providerAccountIdentifier,messageId:id,threadId:message.threadId,processedAt:new Date().toISOString(),matched:eligible.length===1,kind:'cancellation_confirmation',processorVersion:GMAIL_PROCESSOR_VERSION};
      ...
      if(eligible.length===1){
       const t=eligible[0],trialId=t.id!,confirmed={...t,...providerSupport(t.provider),state:'CANCELED_CONFIRMED' as State,verificationRef:{provider:'google' as const,messageId:id,threadId:message.threadId,receivedAt:evidence.receivedAt},evidence:'Authenticated Canva cancellation confirmation received '+new Date(evidence.receivedAt).toISOString()+'.',nextAction:undefined};
       await replace(u,trialId,confirmed);
       const pending=await listAllJobs(j=>j.userId===u&&j.trialId===trialId&&j.status==='PENDING');
       if(pending.length)await db.delete('jobs',pending.map(j=>j.id));
       await audit(u,trialId,'CANCELED_CONFIRMED','Authenticated Canva confirmation email independently verified the provider result; remaining scheduled actions were revoked.');
       await addNotice(u,{kind:'cancellation_verified',title:'Canva cancellation verified',message:'Trialvisor matched an authenticated Canva confirmation email. No complete email body was retained.',severity:'success',trialId,provider:'Canva',actionView:'overview'});
       await notifications.send({userIds:[u],notification:{title:'Cancellation verified',body:'Canva cancellation was verified from authenticated provider evidence.'},data:{trialId,kind:'verified'}});
     ...
     const classification=classifyMessage(message),signal=classification?.signal||null,sourceRecord={providerAccountIdentifier:rec.providerAccountIdentifier,messageId:id,threadId:message.threadId,processedAt:new Date().toISOString(),matched:!!signal,classification:classification?.classification||'NONE',classificationReason:classification?.reason||'No subscription or trial signal found.',kind:'subscription_signal',processorVersion:GMAIL_PROCESSOR_VERSION};
     if(prior)await db.update(table(u,'gmail-sources'),[{id:prior.id,record:{...prior,...sourceRecord}}]);
     else await db.add(table(u,'gmail-sources'),[sourceRecord]);
     if(classification?.classification==='PROMOTIONAL'){
      promotional++;
      const linked=existingTrials.filter(t=>!t.authorizationAt&&t.state==='REVIEW_REQUIRED'&&(t.sourceRef?.messageId===id||(t.supportingSourceRefs||[]).some(x=>x.messageId===id)));
      for(const t of linked){if(!t.id)continue;const next={...t,enrollmentStatus:'PROMOTIONAL' as const,exposureStatus:'EXCLUDED' as const,protectEligible:false,classificationReason:classification.reason,archivedAt:new Date().toISOString()};await replace(u,t.id,next);Object.assign(t,next);await audit(u,t.id,'PROMOTIONAL_EXCLUDED','Processor v5 identified offer-only language without enrollment evidence. The item was removed from exposure and is not Protect eligible.');}
      continue;
     }
     if(!signal)continue;
     signal.sourceRef.accountIdentifier=rec.providerAccountIdentifier;
     if(signal.enrollmentStatus==='ACTIVE')active++;else possible++;
     matched++;if(signal.confidenceBand==='HIGH')high++;else if(signal.confidenceBand==='MEDIUM')medium++;else low++;
     const related=existingTrials.find(t=>sameSubscriptionEntity(t,signal));
     if(related){
      duplicates++;
      if(related.id){
       const stronger=!related.authorizationAt&&related.enrollmentStatus!=='ACTIVE'&&signal.enrollmentStatus==='ACTIVE';
       const next:Trial={...related,...(stronger?{plan:signal.plan,price:signal.price,currency:signal.currency,billingCadence:signal.billingCadence,providerSubscriptionId:signal.providerSubscriptionId,trialEnd:signal.trialEnd,safeDeadline:signal.safeDeadline,plannedExecution:signal.plannedExecution,providerTrialEndDate:signal.providerTrialEndDate,hasExplicitTime:signal.hasExplicitTime,hasDetectedChargeDate:signal.hasDetectedChargeDate,confidence:signal.confidence,confidenceBand:signal.confidenceBand,enrollmentStatus:signal.enrollmentStatus,exposureStatus:signal.exposureStatus,protectEligible:signal.protectEligible,classificationReason:signal.classificationReason,archivedAt:undefined}:{}),supportingSourceRefs:mergeSourceReferences(related,signal)};
       await replace(u,related.id,next);
       Object.assign(related,next);
       await audit(u,related.id,'SUBSCRIPTION_EVIDENCE_ADDED',stronger?'New provider evidence promoted this finding to active enrollment.':'A related Gmail message was attached as supporting evidence instead of creating a duplicate.');
      }
      continue
     }
   ...
   if(ids.size>GMAIL_CANDIDATE_CAP)markCapped();
  const lastResult={mode:Array.from(connectionModes).join('+')||'incremental_no_changes',scanned:scanned-before.scanned,matched:matched-before.matched,created:created-before.created,duplicates:duplicates-before.duplicates,fetchFailures:fetchFailures-before.fetchFailures,classification:{active:active-before.active,possible:possible-before.possible,promotional:promotional-before.promotional},confidence:{high:high-before.high,medium:medium-before.medium,low:low-before.low},incrementalCandidates:incrementalCandidates-before.incrementalCandidates,searchCandidates:searchCandidates-before.searchCandidates,capped:connectionCapped,processorVersion:GMAIL_PROCESSOR_VERSION};
  const now=new Date().toISOString(),record={userId:u,providerAccountIdentifier:rec.providerAccountIdentifier,historyId:nextHistoryCheckpoint(profile.historyId,checkpoint?.historyId,lastResult.fetchFailures),lastSync:now,processorVersion:GMAIL_PROCESSOR_VERSION,lastResult};
  ...
  if(lastResult.fetchFailures>0)await addNotice(u,{kind:'sync_failure',title:'Gmail synchronization needs retry',message:lastResult.fetchFailures+' candidate message'+(lastResult.fetchFailures===1?' could':'s could')+' not be retrieved. Trialvisor preserved the prior history checkpoint so the messages can be retried safely.',severity:'warning',provider:'Google',actionView:'accounts'});
 }
 ...
 return{ok:true,mode,scanned,matched,created,verifiedCancellations,duplicates,fetchFailures,classification:{active,possible,promotional},confidence:{high,medium,low},incrementalCandidates,searchCandidates,capped,processorVersion:GMAIL_PROCESSOR_VERSION};
};
