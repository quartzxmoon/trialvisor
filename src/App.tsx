...
type Snapshot={user:{name?:string;email?:string};account:{initialized:boolean;onboardingComplete:boolean;guidanceShown?:string[]};trials:Trial[];exposure:{confirmed7:number;confirmed30:number;uncertainCount:number;uncertainAmount:number};saved:number;events:number;activity:CustomerActivity[];notifications:PersistentNotice[];connections:Connection[];config:{google:boolean;microsoft:boolean;stripe:boolean;email:boolean};billing:{configured:boolean;plan:'FREE'|'PRO';status:string;cadence:'monthly'|'annual'|null;currentPeriodEnd:string|null;cancelAtPeriodEnd:boolean;hasCustomer:boolean;needsAttention:boolean;canCheckout:boolean;checkoutPending:boolean;reconciliation:'synced'|'stale'|'error';message:string|null};gmailDiscovery?:{processedReferences:number;matchedReferences:number;promotionalReferences?:number;lastSync:string|null;lastResult?:GmailSyncResult|null;verified:boolean}};
...
const dateTime=(s?:string)=>s?new Date(s).toLocaleString(undefined,{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'}):'Not scheduled';
const daysLeft=(s?:string)=>s?Math.max(0,Math.ceil((new Date(s).getTime()-Date.now())/86400000)):999;
...
function App(){
 ...
 const post=async(path:string,body:object={},reloadAfter=true)=>{
  try{
   setBusy(path);
   setError('');
   const sid=getStoredSessionId();
 ...
 const openCheckout=async(cadence:'monthly'|'annual')=>{if(billingRedirectRef.current)return;billingRedirectRef.current=true;try{const r=await post('/api/billing/checkout',{cadence},false);if(r?.url)window.location.assign(r.url);else if(r)setError('Stripe Checkout did not return a secure destination. Please retry.')}finally{billingRedirectRef.current=false}};
 const openPortal=async()=>{if(billingRedirectRef.current)return;billingRedirectRef.current=true;try{const r=await post('/api/billing/portal',{},false);if(r?.url)window.location.assign(r.url);else if(r)setError('Stripe billing management did not return a secure destination. Please retry.')}finally{billingRedirectRef.current=false}};
 ...
 const confirmFinding=async(t:Trial)=>{await post('/api/trials/'+t.id+'/confirm',{});setToast({key:'CONFIRMED',text:t.provider+' confirmed as active subscription.'})};
 const dismissFinding=async(t:Trial,reason:string)=>{await post('/api/trials/'+t.id+'/dismiss',{reason});setToast({key:'DISMISSED',text:t.provider+' finding dismissed and excluded.'})};
...
if(!auth.isSignedIn()&&!snap)return <main className='landing'><nav className='marketing-nav'><img className='wordmark' src='./resources/trialvisor-wordmark.png' alt='Trialvisor'/><div className='nav-proof'><span>Free trials. Under control.</span><a href='./how-it-works'>How it works</a><button className='ghost' onClick={signIn}>Sign in</button></div></nav><section className='hero'><div className='hero-copy'><div className='eyebrow'>FREE-TRIAL PROTECTION</div><h1>Start the trial.<br/><em>Trialvisor remembers the cancellation.</em></h1><p>Find trial and subscription signals, choose what to protect, and let Trialvisor monitor the safest time to act. Cancellation is never authorized by detection alone.</p><div className='hero-actions'><button onClick={signIn} disabled={!!busy}>{busy==='signin'?'Opening secure sign-in...':'Create secure account'}<ArrowRight size={17}/></button><span>Explicit opt-in. Verified outcomes. No dark patterns.</span></div>{error&&<div className='error'>{error}<button className='link-inline' onClick={resetSignIn}>Reset sign-in</button></div>}</div><div className='hero-product'><div className='mini-brand'><img src='./resources/trialvisor-shield.png' alt=''/><span>Example protection view</span></div><strong>What could charge me next?</strong><article><div><b>Canva Pro</b><span>Charges Sep 12</span></div><div><b>$14.99</b><span>Protected</span></div></article><article className='warning-card'><div><b>Adobe Acrobat</b><span>Charges Sep 8</span></div><div><b>$29.99</b><span>Needs review</span></div></article><div className='preview-foot'><ShieldCheck size={16}/> Safe Cancel Deadline monitored</div></div></section><section className='promise-grid'><article><span>01</span><h3>Discover</h3><p>Trialvisor searches connected inboxes for trial starts, renewal notices, and upcoming charges.</p></article><article><span>02</span><h3>Protect</h3><p>You explicitly authorize protection for a specific trial. Silence only means cancel after that authorization.</p></article><article><span>03</span><h3>Verify</h3><p>Trialvisor checks the provider result and records evidence before calling a cancellation successful.</p></article></section><section className='trust-strip'><span><Check/> Privacy-minimized inbox analysis</span><span><Check/> Revocable connected accounts</span><span><Check/> Auditable cancellation decisions</span></section><div className='landing-links'><a href='./how-it-works'>How Trialvisor works</a><a href='./providers'>Provider support</a><a href='./security'>Security</a><a href='./subprocessors'>Subprocessors</a></div>{toast&&<ToastView toast={toast} close={()=>setToast(null)}/>}</main>;
 ...
 const activeTrials=trials.filter(t=>t.state!=='DISMISSED'&&t.enrollmentStatus!=='PROMOTIONAL'&&t.kind!=='PROMOTIONAL');
 const confirmedTrials=activeTrials.filter(t=>(t.enrollmentStatus||t.kind)==='ACTIVE'||(!t.enrollmentStatus&&!t.kind));
 const possibleTrials=activeTrials.filter(t=>(t.enrollmentStatus||t.kind)==='POSSIBLE');
 ...
 const actionTrials=activeTrials.filter(t=>['CANCELLATION_NEEDS_USER','CANCELLATION_FAILED'].includes(t.state));
 const soonTrials=activeTrials.filter(t=>!['CANCELED_CONFIRMED','KEEP_REQUESTED'].includes(t.state)&&(t.enrollmentStatus||t.kind)==='ACTIVE'&&t.exposureStatus==='CONFIRMED'&&daysLeft(t.trialEnd)<=7);
 const exposure7=snap.exposure?.confirmed7||0;
 const exposure30=snap.exposure?.confirmed30||0;
 const ordered=[...activeTrials].sort((a,b)=>{const aTime=a.safeDeadline?new Date(a.safeDeadline).getTime():(a.trialEnd?new Date(a.trialEnd).getTime():9999999999999);const bTime=b.safeDeadline?new Date(b.safeDeadline).getTime():(b.trialEnd?new Date(b.trialEnd).getTime():9999999999999);return aTime-bTime;});
 ...
 <section className='health'><div><span>PROTECTION HEALTH</span><b>{confirmedTrials.length} confirmed subscription{confirmedTrials.length===1?'':'s'}</b><small>{protectedTrials.length} protected · {possibleTrials.length} pending review · {soonTrials.length} may charge within 7 days</small></div><strong className={actionTrials.length?'urgent-pill':'good-pill'}>{actionTrials.length?`${actionTrials.length} action required`:'Protection stable'}</strong></section>
 {actionTrials.length>0&&<section className='priority-section'><div className='section-label urgent'><AlertTriangle/>ACTION REQUIRED</div>{actionTrials.map(t=><TrialCard key={t.id} t={t} primaryLabel='Review required step' onPrimary={()=>setModal({kind:'decision',trial:t})} onConfirm={confirmFinding} onDismiss={dismissFinding}/>)}</section>}
...
{view==='accounts'&&<section><header className='page-head'><div><div className='eyebrow'>CONNECTED ACCOUNTS</div><h2>Subscription discovery sources</h2><p>Your Trialvisor login is separate from the inboxes you authorize for discovery.</p></div></header>{error&&<div className='error' role='alert'>{error}</div>}<div className='account-grid'>{['Google','Microsoft'].map(provider=>{const c=snap.connections.find(x=>x.provider===provider&&(provider!=='Google'||x.mode==='PRODUCTION'));const configured=provider==='Google'?snap.config.google:snap.config.microsoft;return <article className='account-card' key={provider}><div className='account-title'><div className={'provider '+provider.toLowerCase()}>{provider[0]}</div><div><h3>{provider}</h3><span>{c?.providerAccountIdentifier||'Not connected'}</span></div><StatusBadge state={c?.status||'DISCONNECTED'}/></div><dl><div><dt>Connection</dt><dd>{c?.status||'Not connected'}</dd></div><div><dt>Last sync</dt><dd>{c?.lastSync?dateTime(c.lastSync):'Never'}</dd></div><div><dt>Discovery</dt><dd>{provider==='Google'&&c?(snap.gmailDiscovery?.verified?'Real mailbox finding verified':'Connected - real finding still pending validation'):'No validated findings yet'}</dd></div></dl><div className='account-actions'>{provider==='Google'&&configured?(c?<><button onClick={syncGoogle} disabled={!!busy}><RefreshCw size={16}/>{busy==='/api/gmail/sync'?'Syncing…':'Sync'}</button><button className='secondary' onClick={connectGoogle} disabled={!!busy}>Reconnect</button><button className='text-danger' onClick={()=>post('/api/oauth/google/disconnect')} disabled={!!busy}>Disconnect</button></>:<button onClick={connectGoogle} disabled={!!busy}>Connect Google</button>):<button className='secondary' disabled>{configured?'Connection coming next':'Production OAuth not configured'}</button>}</div></article>})}</div>{google&&<div className='validation-note'><Eye/><div><b>Real Gmail discovery validation</b><span>{snap.gmailDiscovery?.verified?'Verified with at least one persisted mailbox finding.':`Still open. ${snap.gmailDiscovery?.processedReferences||0} message references processed and ${snap.gmailDiscovery?.matchedReferences||0} matched so far.`}</span>{snap.gmailDiscovery?.lastResult&&<small className='validation-template'>Last sync: {snap.gmailDiscovery.lastResult.mode.replace(/_/g,' ')} · {snap.gmailDiscovery.lastResult.scanned} scanned · {snap.gmailDiscovery.lastResult.matched} matched · {snap.gmailDiscovery.lastResult.created} new · {snap.gmailDiscovery.lastResult.duplicates} duplicates · {snap.gmailDiscovery.lastResult.fetchFailures} retrieval failures.</small>}{!snap.gmailDiscovery?.verified&&<small className='validation-template'>Controlled check: from another address, email the connected Gmail account with subject “Your free trial has started” and body “Your trial ends September 10, 2026. You will be charged $14.99/month after your trial.” Then select Sync.</small>}</div></div>}</section>}
 ...
 {view==='billing'&&<section>
  <header className='page-head'><div><div className='eyebrow'>PLAN & BILLING</div><h2>Protection that stays easy to cancel</h2><p>Stripe-hosted checkout and account management with server-verified access.</p></div></header>
  {error&&<div className='error' role='alert'>{error}</div>}
  {snap.billing.reconciliation==='error'&&<div className='error' role='alert'>{snap.billing.message}</div>}
  <div className='billing-card'><div><span className='plan-pill'>CURRENT PLAN</span><h3>Trialvisor {snap.billing.plan==='PRO'?'Pro':'Free'}</h3><p>{snap.billing.plan==='PRO'?`Paid protection is ${label(snap.billing.status).toLowerCase()} on the ${snap.billing.cadence==='annual'?'yearly':'monthly'} plan${snap.billing.cancelAtPeriodEnd?' and is scheduled to end at the close of the paid period':''}.`:snap.billing.message||'Your account is on the Free plan. Paid access activates only after a signed Stripe subscription event.'}</p>{snap.billing.currentPeriodEnd&&<small>{snap.billing.cancelAtPeriodEnd?'Access remains through':'Current period ends'} {day(snap.billing.currentPeriodEnd)}</small>}</div>
   {snap.billing.plan==='PRO'?<button onClick={openPortal} disabled={!!busy}>{busy==='/api/billing/portal'?'Opening…':'Manage billing'}</button>:snap.billing.checkoutPending?<div className='billing-actions single'><button onClick={()=>openCheckout(snap.billing.cadence||'monthly')} disabled={!snap.billing.canCheckout||!!busy}>{busy==='/api/billing/checkout'?'Opening…':`Continue ${snap.billing.cadence==='annual'?'yearly':'monthly'} Checkout`}</button><small>Your existing secure Stripe Checkout will reopen; a second subscription will not be created.</small></div>:snap.billing.canCheckout?<div className='billing-actions'><button onClick={()=>openCheckout('monthly')} disabled={!!busy}>{busy==='/api/billing/checkout'?'Opening…':'Choose monthly'}</button><button className='secondary' onClick={()=>openCheckout('annual')} disabled={!!busy}>{busy==='/api/billing/checkout'?'Opening…':'Choose annual'}</button>{snap.billing.hasCustomer&&<button className='secondary' onClick={openPortal} disabled={!!busy}>Manage billing</button>}<small>Exact prices are shown in secure Stripe Checkout.</small></div>:<div className='billing-actions single'><div className='billing-state-warning' role='alert'>{snap.billing.message||'Billing status requires attention before another Checkout can begin.'}</div>{snap.billing.hasCustomer&&<button className='secondary' onClick={openPortal} disabled={!!busy}>Manage billing</button>}</div>}
  </div><div className='billing-trust'><ShieldCheck/><div><b>Server-owned billing status</b><span>Trialvisor reconciles your plan with Stripe and grants Pro only from verified server-side subscription state. A browser redirect alone never grants paid access.</span></div></div>
 </section>}
...
function TrialCard({t,primaryLabel,onPrimary,onConfirm,onDismiss}:{t:Trial;primaryLabel?:string;onPrimary?:()=>void;onConfirm?:(t:Trial)=>void;onDismiss?:(t:Trial,reason:string)=>void}){
  const isAssist=t.state==='CANCELLATION_NEEDS_USER',isCanceled=t.state==='CANCELED_CONFIRMED',isPossible=(t.enrollmentStatus||t.kind)==='POSSIBLE';
  return <article className={'trial '+(isAssist?'trial-assist ':'')+(isCanceled?'trial-canceled':'')}><div className='trial-top'><div className='provider-icon'>{t.provider[0]}</div><div className='trial-name'><h4>{t.provider}<span>{t.plan}</span></h4><div className='chips'><StatusBadge state={isPossible?'NEEDS_CONFIRMATION':t.state}/><i>{t.sourceRef?'Inbox detected':'Manual'}</i><i>{t.confidenceBand||Math.round(t.confidence*100)+'%'} confidence</i></div></div><div className='charge'><b>{t.price>0?money(t.price):'Unconfirmed'}</b><span>{t.price>0?'/ month':'amount'}</span></div></div><div className='dates'><div><span>TRIAL ENDS</span><b>{t.providerTrialEndDate||(t.trialEnd?day(t.trialEnd):'Needs confirmation')}</b></div><div><span>SAFE CANCEL DEADLINE</span><b>{t.safeDeadline?dateTime(t.safeDeadline):'Not scheduled'}</b></div><div><span>PLANNED ACTION</span><b>{t.plannedExecution?dateTime(t.plannedExecution):'Awaiting confirmation'}</b></div><div><span>CANCELLATION SUPPORT</span><b>{supportLabel(t)}</b></div></div>{isPossible&&<div className='assist' style={{background:'#fef3c7',borderColor:'#f59e0b'}}><AlertTriangle size={18} color='#d97706'/><div><b>Needs Review</b><span>Trialvisor isn't sure this is an active subscription.</span></div></div>}{isAssist&&<div className='assist'><AlertTriangle size={18}/><div><b>Action required</b><span>{t.nextAction||'The provider requires your participation before Trialvisor can continue.'}</span></div></div>}{t.evidence&&<div className='evidence'><CheckCircle2 size={18}/><div><b>Verified cancellation</b><span>{t.evidence}</span></div></div>}<div className='actions'>{isPossible&&t.state==='REVIEW_REQUIRED'?(<div className='review-actions' style={{display:'flex',gap:'0.5rem',flexWrap:'wrap'}}><button className='compact' onClick={()=>onConfirm?.(t)}><Check size={15}/>Yes, this is mine</button><button className='secondary compact' onClick={()=>onDismiss?.(t,'not_subscription')}>Not a subscription</button><button className='secondary compact' onClick={()=>onDismiss?.(t,'offer_only')}>This is only an offer</button></div>):(<>
{primaryLabel&&onPrimary&&<button onClick={onPrimary}>{primaryLabel}<ChevronRight size={16}/></button>}
{t.state==='REVIEW_REQUIRED'&&<button className='secondary compact' onClick={()=>onDismiss?.(t,'dismissed')}>Dismiss</button>}
{isCanceled&&<span className='success'>Cancellation independently verified</span>}{t.state==='KEEP_REQUESTED'&&<span className='kept'>Kept - scheduled cancellation revoked and audited.</span>}</>)}</div><details className='trial-activity'><summary>Activity timeline</summary><TrialActivity activity={t.activity||[]}/></details></article>;
}
