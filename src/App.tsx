...
type Trial={id:string;provider:string;plan:string;price:number;currency:string;trialEnd:string;safeDeadline:string;plannedExecution:string;state:string;tier:string;support?:'GUIDED'|'NOT_SUPPORTED';confidence:number;confidenceBand?:'HIGH'|'MEDIUM'|'LOW';evidence?:string;nextAction?:string;authorizationAt?:string;providerActionAt?:string;cancellationUrl?:string;sourceRef?:{provider:'google'|'microsoft';messageId:string;threadId:string;receivedAt?:string};verificationRef?:{provider:'google';messageId:string;threadId:string;receivedAt?:string};activity?:CustomerActivity[]};
type Connection={id?:string;provider:string;providerAccountIdentifier?:string;status:string;lastSync:string;mode:string};
type Snapshot={user:{name?:string;email?:string};account:{initialized:boolean;onboardingComplete:boolean;guidanceShown?:string[]};trials:Trial[];saved:number;events:number;activity:CustomerActivity[];notifications:PersistentNotice[];connections:Connection[];config:{google:boolean;microsoft:boolean;stripe:boolean;email:boolean};billing:{configured:boolean;plan:'FREE'|'PRO';status:string;cadence:'monthly'|'annual'|null;currentPeriodEnd:string|null;cancelAtPeriodEnd:boolean;hasCustomer:boolean};gmailDiscovery?:{processedReferences:number;matchedReferences:number;lastSync:string|null;verified:boolean}};
...
function App(){
 ...
 const syncGoogle=async()=>{
  ...
  if(r.verifiedCancellations>0)setToast({key:'SYNC_RESULT',text:'Sync complete. Trialvisor independently verified '+r.verifiedCancellations+' Canva cancellation'+(r.verifiedCancellations===1?'':'s')+' from authenticated provider email. Complete message bodies were not retained.'});
  else if(r.created>0){
 ...
 const soonTrials=trials.filter(t=>!['CANCELED_CONFIRMED','KEEP_REQUESTED'].includes(t.state)&&daysLeft(t.trialEnd)<=7);
 const exposure7=trials.filter(t=>!['CANCELED_CONFIRMED','KEEP_REQUESTED'].includes(t.state)&&daysLeft(t.trialEnd)<=7).reduce((s,t)=>s+t.price,0);
 const exposure30=trials.filter(t=>!['CANCELED_CONFIRMED','KEEP_REQUESTED'].includes(t.state)&&daysLeft(t.trialEnd)<=30).reduce((s,t)=>s+t.price,0);
