...
type GmailSyncResult={mode:string;scanned:number;matched:number;created:number;duplicates:number;fetchFailures:number;confidence:{high:number;medium:number;low:number};incrementalCandidates:number;searchCandidates:number;capped:boolean;processorVersion:number};
type Snapshot={user:{name?:string;email?:string};account:{initialized:boolean;onboardingComplete:boolean;guidanceShown?:string[]};trials:Trial[];saved:number;events:number;activity:CustomerActivity[];notifications:PersistentNotice[];connections:Connection[];config:{google:boolean;microsoft:boolean;stripe:boolean;email:boolean};billing:{configured:boolean;plan:'FREE'|'PRO';status:string;cadence:'monthly'|'annual'|null;currentPeriodEnd:string|null;cancelAtPeriodEnd:boolean;hasCustomer:boolean};gmailDiscovery?:{processedReferences:number;matchedReferences:number;lastSync:string|null;lastResult?:GmailSyncResult|null;verified:boolean}};
